...
void loop() {
  ...
  while(question < 4){
	  if(question == 1){
		  Serial.println("Trouvez-vous ce projet intéressant?");
  	}
	  if(question == 2){
		  Serial.println("Connaissez vous Chronos?");
	  }
	  if(question == 3){
		  Serial.println("Avez-vous un chien?");
  	}
  ...
  // Loop through the output tensor values from the model
    ...
    if(question == 4){
		  while(1)
      {
        //Le plus simple pour que le programme laisse penser que le travail est fini je n'ai pas réfléchis à le faire de manière propre
      }
  	}
  }
}
